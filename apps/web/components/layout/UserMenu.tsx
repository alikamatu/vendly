...
export default function UserMenu() {
  ...
  const navLinks = [
    { label: "My Profile", href: "/dashboard/settings/profile", icon: User, show: true },
    { label: "Dashboard", href: "/dashboard", icon: LayoutDashboard, show: isSeller || isAdmin },
    { label: "Manage Products", href: "/dashboard/products", icon: Package, show: isSeller },
    { label: "Orders", href: isSeller ? "/dashboard/orders" : "/orders", icon: ShoppingBag, show: true },
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <ChevronDown size={14} className={clsx("text-muted transition-transform duration-300", isOpen && "rotate-180")} />
      </button>
            ...
            >
              <div className={clsx("p-2", isMobile ? "pb-10 px-4" : "p-2")}>
                ...
                <div className={clsx(
                  ...
                  isMobile ? "py-6 border-b border-border/50" : "p-4 bg-surface/50 rounded-2xl border border-border/20"
                )}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs font-black uppercase tracking-widest text-foreground">{user.full_name}</p>
                      <p className="text-[10px] text-muted mt-0.5">{user.email}</p>
                    </div>
                  </div>
                  {isMobile && (
                    <button onClick={() => setIsOpen(false)} className="p-2 bg-surface rounded-full text-muted active:scale-90 transition-transform">
                      <X size={20} />
                    </button>
                  )}
                </div>
                ...
                {isSeller && !isMobile && (
                  <div className="px-4 py-2 mb-4 flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-primary bg-primary/5 rounded-xl border border-primary/10">
                    ...
                    <Link href={`/s/${user.seller_profile?.store_link}`} onClick={() => setIsOpen(false)} className="flex items-center gap-1 hover:underline">
                ...
                {/* Nav Links */}
                <div className={clsx("space-y-1", isMobile ? "mt-4 space-y-2" : "")}>
                  {navLinks.filter(link => link.show).map((link) => (
                    ...
                    >
                      <link.icon size={isMobile ? 22 : 18} className={clsx(
                        "transition-transform group-hover:scale-110 group-active:scale-95",
                        isMobile ? "text-primary/70" : "text-muted group-hover:text-primary"
                      )} />
                ...
                <div className={clsx(
                  "mt-2 pt-2",
                  isMobile ? "mt-6 pt-4 border-t border-border/50" : "border-t border-border/20"
                )}>
                  <button
                    onClick={() => {
                      logout();
                  ...
                  >
                    <LogOut size={isMobile ? 22 : 18} className="group-hover:translate-x-1 transition-transform" />
                    <span>Sign Out</span>
                  </button>
                </div>
              </div>
